...
class AlarmTest {
    ...
    void handleEventWhenActivated() throws IOException {
        ...
        EventHandler handler = new AlarmDecorator(new DoorEventHandler(smartHome));
        smartHome.getAlarm().getState().activate("SOME_SECRET_CODE");
        handler.handleEvent(event1);
        Assert.assertEquals(smartHome.getAlarm().getState().getType(), State.StateType.ALARM);
    ...
    void handleEventWhenDeactivated() throws IOException {
        ...
        EventHandler handler = new AlarmDecorator(new DoorEventHandler(smartHome));
        handler.handleEvent(event1);
        Assert.assertEquals(smartHome.getAlarm().getState().getType(), State.StateType.DEACTIVATED);
        Collection<Room> rooms = smartHome.getRooms();
        for (Room room: rooms) {
    ...
    void handleEventWhenAlarm() throws IOException {
        ...
        SensorEvent event1 = new SensorEvent(SensorEventType.DOOR_OPEN, "1");
        EventHandler handler = new AlarmDecorator(new DoorEventHandler(smartHome));
        smartHome.getAlarm().getState().setAlarmState();
        handler.handleEvent(event1);
        Assert.assertEquals(smartHome.getAlarm().getState().getType(), State.StateType.ALARM);
        Collection<Room> rooms = smartHome.getRooms();
        for (Room room: rooms) {
